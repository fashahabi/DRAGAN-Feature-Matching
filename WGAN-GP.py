...
# Define WGAN-GP and it's components 
...
class WGAN(object):
    ...
    def WGAN_penalty(self, N, X_real, X_fake):
        X_real = tf.cast(X_real, tf.float32)
        X_fake = tf.cast(X_fake, tf.float32)
        # Calculate the norm of discriminator's gradient
        alpha = tf.random.normal([N, self.input_win+self.output_win, 1], 0.0, 1.0)
        diff = X_fake - X_real
        ...
        with tf.GradientTape() as gp_tape:
            ...
            disc_out = self.disc(interpolated, training=True)

        grads_gp = gp_tape.gradient(disc_out, [interpolated])[0]
        norm = tf.sqrt(tf.reduce_sum(tf.square(grads_gp), axis=[1, 2]))
    ...
    def train(self, segment_train, y_target, historical_price):
        ...
        for step in range(1):
            ...
            with tf.GradientTape() as disc_tape:                
                ...
                penalty_term = self.WGAN_penalty(N, X_real, X_fake)
    ...
    def test(self, segment_test):
        ...
        # test_list = []
        # index_e = []
        # for i in range(125, 271, 5):
        #     index_e.append(i)
        #     self.gen.load_weights('WGAN_%d.h5'% i)
        ...
        # plt.show()
        # exit(0)
        # self.gen.load_weights('WGAN_150.h5') # uncomment this line when trained model is available
...
if __name__ == '__main__':
    ...
    # Define WGAN-GP and initialize
    feature_m = segment_train.shape[2]
    WGAN_model = WGAN(input_win, output_win, feature_m)
    ...
    for iteration in range(iterations):
        dis_loss_, gen_loss_, y_predict = WGAN_model.train(segment_train, y_train, h_prices_train)
        ...
        if (iteration % 5) == 0 and iteration> 115:
            print(iteration)
            WGAN_model.gen.save_weights('WGAN_%d.h5' % iteration)
            #tf.keras.models.save_model(WGAN_model.gen, 'WGAN_%d.h5' % iteration, save_format='tf')
    
    #WGAN_model.gen.load_weights('WGAN_20.h5')
    WGAN_model.gen.save_weights('WGAN_last.h5')
    print('\n=========================================================================')
    print('   RMSE_scaled', ' RMSE', '   MAE', '  R2-Score')
    
    if output_win < 2:
        y_predict_test = WGAN_model.test(segment_test)
        ...
        y_predict_train = WGAN_model.test(segment_train)
